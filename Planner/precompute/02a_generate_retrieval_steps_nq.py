...
GENERATOR_MODEL = os.environ.get("GENERATOR_MODEL", "Qwen/Qwen2.5-7B-Instruct")
GENERATOR_MODEL_PATH = os.environ.get("GENERATOR_MODEL_PATH", "/mnt/raid6/skbaek1223/models/Qwen/Qwen2.5-7B-Instruct")
...
NUM_WORKERS = int(os.environ.get("NUM_WORKERS", "40"))
CHUNK_SIZE = int(os.environ.get("CHUNK_SIZE", "2000"))
MAX_TOKENS = 128

DATA_DIR = Path(__file__).parent.parent.parent / "data" / "precompute"
IN_PATH = DATA_DIR / "nq_all.jsonl"
OUT_PATH = DATA_DIR / "nq_all_with_steps.jsonl"

_clients: list[OpenAI] = []
_client_counter = 0

SYSTEM_PROMPT = """You are an information retrieval planning expert. Given a question, generate a single concrete retrieval step required to find the answer.

Write exactly one step. No other text.

Example:

Question: who holds the most women's wimbledon titles

Find who holds the most women's titles at Wimbledon."""
...
def _main_process():
    if not IN_PATH.exists():
        raise FileNotFoundError(f"{IN_PATH} not found. Run 01_a_load_nq.py first.")
