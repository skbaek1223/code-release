...
def main():
    ...
    } for item, guide, prompt in zip(filtered_data, retrieval_guides, input_list)]
    ...
    def llm_chat_batch(user_prompts: List[str], sp: SamplingParams) -> List[str]:
        prompts = [
    ...
    while True:
        ...
        extractor_raw = llm_chat_batch(extractor_prompts, short_sampling)
        extracted_facts = []
        ...
        evaluator_raw_partial = (llm_chat_batch(evaluator_prompts, short_sampling)
                                 if evaluator_prompts else [])
