"""Re-Guide all-datasets launcher preset for Qwen3-4B (non-thinking mode).

Thin wrapper around run_all_datasets.py: injects Qwen3 recommended sampling
defaults and context window suited to the 4B model on a single A6000
...
Examples:
    ...
    python run_all_datasets_qwen3_4b.py --gpus 2,3 --retriever_gpus 0,1

    # single dataset on one GPU
    python run_all_datasets_qwen3_4b.py --gpus 3 --retriever_gpus 0,1 --dataset musique

    # force TP=2 across 6,7 (sequential, one dataset uses both GPUs)
    python run_all_datasets_qwen3_4b.py --gpus 6,7 --retriever_gpus 4,5 \\
...
# 4B is small: use the same conservative values as Llama-8B.
BUDGET_SINGLEHOP = "30"
BUDGET_MULTIHOP = "45"
