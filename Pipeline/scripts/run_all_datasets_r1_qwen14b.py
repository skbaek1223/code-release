"""Re-Guide all-datasets launcher preset for DeepSeek-R1-Distill-Qwen-14B.

Thin wrapper around run_all_datasets.py: injects DeepSeek-R1 recommended
sampling defaults (temperature=0.6) and context window suited to the 14B
model on a single A6000 (max_model_len=32768, max_new_tokens=16384). The
base Qwen2.5 has 131k native context, so larger windows are also valid
...
Examples:
    ...
    python run_all_datasets_r1_qwen14b.py --gpus 6,7 --retriever_gpus 4,5

    # single dataset on one GPU
    python run_all_datasets_r1_qwen14b.py --gpus 2 --retriever_gpus 0,1 --dataset triviaqa,2wiki
...
# Qwen-14B preset: skip triviaqa_a/_b variants (run plain triviaqa only).
