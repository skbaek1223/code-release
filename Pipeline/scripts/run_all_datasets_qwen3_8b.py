"""Re-Guide all-datasets launcher preset for Qwen3-8B (non-thinking mode).

Thin wrapper around run_all_datasets.py: injects Qwen3 recommended sampling
defaults and context window suited to the 8B model on a single A6000
...
Note: the fine-tuned planner checkpoint (planner-5050-5000/final_merged) is
also Qwen3-8B based and is shared across all Qwen3 model runs.
...
Examples:
    ...
    python run_all_datasets_qwen3_8b.py --gpus 6,7 --retriever_gpus 0,1

    # single dataset on one GPU
    python run_all_datasets_qwen3_8b.py --gpus 3 --retriever_gpus 0,1 --dataset musique

    # force TP=2 across 6,7 (sequential, one dataset uses both GPUs)
    python run_all_datasets_qwen3_8b.py --gpus 6,7 --retriever_gpus 4,5 \\
...
# Word-budget for the Reasoning Guide's Insufficient message.
# 8B is mid-size: same conservative values as Llama-8B.
BUDGET_SINGLEHOP = "30"
BUDGET_MULTIHOP = "45"
...
def _build_cmd_with_budget(args, dataset_name, data_file, split, retriever_url):
    ...
    cmd += ["--budget_base", budget, "--thinking_mode"]
