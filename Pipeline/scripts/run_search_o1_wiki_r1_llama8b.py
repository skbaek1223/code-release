"""Search-o1 (wiki / FlashRAG e5) launcher preset for DeepSeek-R1-Distill-Llama-8B.
...
the 8B model on a single A6000 (max_model_len=32768, max_new_tokens=16384).
The model has 131k native context via llama3 RoPE scaling, so larger
windows are also valid if KV cache budget allows.
...
Examples:
    ...
    python run_search_o1_wiki_r1_llama8b.py --gpus 6,7 --retriever_gpus 4,5

    # single dataset on one GPU
    python run_search_o1_wiki_r1_llama8b.py --gpus 3 --retriever_gpus 0,1 --dataset 2wiki,musique
