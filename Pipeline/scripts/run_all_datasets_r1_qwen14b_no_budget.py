"""Re-Guide ablation launcher (Qwen-14B): reasoning guide WITHOUT budget hint.
...
run_all_datasets_r1_qwen14b.py. Output directories get a ".no_budget" suffix
so they don't collide with full-pipeline runs.

Examples:
    # 8 datasets, 2 at a time (1 GPU per dataset)
    python run_all_datasets_r1_qwen14b_no_budget.py --gpus 6,7 --retriever_gpus 0,1

    # single dataset on one GPU
    python run_all_datasets_r1_qwen14b_no_budget.py --gpus 2 --retriever_gpus 0,1 --dataset nq,ambigqa,hotpotqa,musique
"""
import os
import sys
